 ...
 *
 * Set AUTO_BASE_PATH environment variable to your subdirectory path (e.g., /nextaidrawio)
 */
export function getBasePath(): string {
    // Read from environment variable (must start with AUTO_ to be available on client)
    const basePath = process.env.AUTO_BASE_PATH || ""
    if (basePath && !basePath.startsWith("/")) {
        console.warn("AUTO_BASE_PATH should start with /")
    }
    return basePath
}
